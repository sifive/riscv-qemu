...
struct rvv_reg dst_vl[] = {
    {
        .float64 = {
            ...
            {0xc0154afd6a012e31, 0x0000000000000000,},
        },
        .float32 = {
            {0x483471f7, 0x46f2e02b, 0x4785dc35, 0x47ad69d9, },
            {0x483471f7, 0x46f2e02b, 0x7f7fffff, 0x00000000, },
        },
        .float16 = {
            {0x4aee, 0x40aa, 0x4524, 0x46a9, 0x4a65, 0x404e, 0x44bf, 0x4626, },
            {0x4aee, 0x40aa, 0x4524, 0x46a9, 0x4a65, 0x404e, 0x44bf, 0x0000, },
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        .float64 = {
            ...
            {0xc0154afd6a012e31, 0x1111111111111111,},
        },
        .float32 = {
            {0x483471f7, 0x11111111, 0x4785dc35, 0x11111111, },
            {0x483471f7, 0x11111111, 0x7f7fffff, 0x11111111, },
        },
        .float16 = {
            {0x4aee, 0x1111, 0x4524, 0x1111, 0x4a65, 0x1111, 0x44bf, 0x1111, },
            {0x4aee, 0x1111, 0x4524, 0x1111, 0x4a65, 0x1111, 0x44bf, 0x1111, },
        },
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vf_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0][0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vf_32_vm(src0[i].float32[0], src1[i].float32[0][0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vf_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[0][0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vf_64_vm(src0[i].float64[0], src1[i].float64[0][0],
