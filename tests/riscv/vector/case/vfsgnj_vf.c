...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float32 = {
            {0xc83471f7, 0xc6f2e02b, 0xc785dc35, 0xc7ad69d9, },
            {0xc83471f7, 0xc6f2e02b, 0xff7fffff, 0x00000000, },
        },
        .float16 = {
            {0xcaee, 0xc0aa, 0xc524, 0xc6a9, 0xca65, 0xc04e, 0xc4bf, 0xc626, },
            {0xcaee, 0xc0aa, 0xc524, 0xc6a9, 0xca65, 0xc04e, 0xc4bf, 0x0000, },
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        ...
        .float32 = {
            {0xc83471f7, 0x11111111, 0xc785dc35, 0x11111111, },
            {0xc83471f7, 0x11111111, 0xff7fffff, 0x11111111, },
        },
        .float16 = {
            {0xcaee, 0x1111, 0xc524, 0x1111, 0xca65, 0x1111, 0xc4bf, 0x1111, },
            {0xcaee, 0x1111, 0xc524, 0x1111, 0xca65, 0x1111, 0xc4bf, 0x1111, },
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .float64 = {
            ...
            {0x1111111111111111, 0x40417456836cfe7b,},
        },
        .float32 = {
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vf_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0][0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vf_32_vm(src0[i].float32[0], src1[i].float32[0][0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vf_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[0][0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vf_64_vm(src0[i].float64[0], src1[i].float64[0][0],
