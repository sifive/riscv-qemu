...
struct rvv_reg src0[] = {
    {
        .float64 = {
            ...
            {0x40154afd6a012e31, 0x7ff1000000000000,},
        },
        .float32 = {
            {0x7fc00000, 0x46f2e02b, 0xc785dc35, 0x7fc00000, },
            {0xc843ca22, 0x7fc00000, 0xff7fffff, 0x481eefee, },
        },
        .float16 = {
            {0x7e00, 0x40aa, 0x7e00, 0x46a9, 0x4a65, 0x7e00, 0xc4bf, 0x4626, },
            {0x4aee, 0x40aa, 0xc524, 0x46a9, 0x7e00, 0x404e, 0xc4bf, 0x4626, },
        },
    },
};
...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float32 = {
            {0x00010000, 0x00000001, 0x0000001, 0x00000001,},
        },
        .float16 = {
            {0x0100, 0x0100, 0x0001, 0x0101, 0x0101, 0x0101, 0x0100, 0x0001,},
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        ...
        .float16 = {
            {0x1100, 0x1100, 0x1101, 0x1101, 0x1101, 0x1101, 0x1100, 0x1101,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        ...
        .float32 = {
            {0x00011111, 0x00001111, 0x00001111, 0x00011111,},
        },
        .float16 = {
            {0x0111, 0x0111, 0x0011, 0x0111, 0x0111, 0x0111, 0x0111, 0x0111,},
        },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmford_vf_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0][0], res.float32[0], pad.fixu32[0]);
        result_compare_s32(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmford_vf_32_vm(src0[i].float32[0], src1[i].float32[0][0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmford_vf_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[1][0], res.float64[0], pad.fixu64);
        result_compare_s64(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmford_vf_64_vm(src0[i].float64[0], src1[i].float64[1][0],
