...
struct rvv_reg dst_vl[] = {
    {
        .float32 = {
            {0x7fffffff, 0x80000000, 0x0000000e, 0xffffffe9,},
            {0x00000005, 0xffffffdd, 0x0000000e, 0x00000000,},
        },
        .float16 = {
            {0x7fff, 0x7970, 0x8000, 0x7fff, 0x8000, 0x7fff, 0x8000, 0x7fff,},
            {0x7fff, 0x7970, 0x8000, 0x7fff, 0x8000, 0x7fff, 0x8000, 0x0000,},
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        .float32 = {
            {0x7fffffff, 0x11111111, 0x0000000e, 0x11111111,},
            {0x00000005, 0x11111111, 0x0000000e, 0x11111111,},
        },
        .float16 = {
            {0x7fff, 0x1111, 0x8000, 0x1111, 0x8000, 0x1111, 0x8000, 0x1111,},
            {0x7fff, 0x1111, 0x8000, 0x1111, 0x8000, 0x1111, 0x8000, 0x1111,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .float32 = {
            {0x11111111, 0x80000000, 0x11111111, 0xffffffe9,},
            {0x11111111, 0xffffffdd, 0x11111111, 0xffffffe9,},
        },
        .float16 = {
            {0x1111, 0x7970, 0x1111, 0x7fff, 0x1111, 0x7fff, 0x1111, 0x7fff,},
            {0x1111, 0x7970, 0x1111, 0x7fff, 0x1111, 0x7fff, 0x1111, 0x7fff,},
        },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfncvt.x.f.v\n");


    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfncvt_x_f_v_16(vlmax_16 - 1, src0[i].wfloat32[0],
                src1[i].float16[0], res.float16[0], pad.fixu16[0]);
        result_compare_s16_lmul(res.float16[0], dst_vl[i].float16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfncvt_x_f_v_16_vm(src0[i].wfloat32[0], src1[i].float16[0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfncvt_x_f_v_32(vlmax_32 - 1, src0[i].wfloat64[0],
                src1[i].float32[0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfncvt_x_f_v_32_vm(src0[i].wfloat64[0], src1[i].float32[0],
