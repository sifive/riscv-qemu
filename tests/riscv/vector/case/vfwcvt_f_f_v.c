...
struct rvv_reg dst_vl[] = {
    {
        ...
        .wfloat32 = {
            ...
            {0x415dc000, 0xc77fe000, 0xc0a48000, 0x477fe000, 0x414ca000, 0x4009c000, 0xc097e000, 0x00000000,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .wfloat64 = {
            {0x1111111111111111, 0x40de5c0560000000, 0x1111111111111111, 0x40f5ad3b20000000,},
            {0x1111111111111111, 0x40de5c0560000000, 0x1111111111111111, 0x40f5ad3b20000000,},
        },
        .wfloat32 = {
            {0x11111111, 0x40154000, 0x11111111, 0x40d52000, 0x11111111, 0x4009c000, 0x11111111, 0x40c4c000,},
            {0x11111111, 0xc77fe000, 0x11111111, 0x477fe000, 0x11111111, 0x4009c000, 0x11111111, 0x40c4c000,},
        },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfwcvt_f_f_v_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0], res.wfloat64[0], pad.fixu32[0]);
        result_compare_s64_lmul(res.wfloat64[0], dst_vl[i].wfloat64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfwcvt_f_f_v_32_vm(src0[i].float32[0], src1[i].float32[0],
