...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float16 = {
            {0x0101, 0x0100, 0x0001, 0x0100, 0x0101, 0x0100, 0x0000, 0x0000,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        ...
        .float16 = {
            {0x0111, 0x0111, 0x0011, 0x0111, 0x0111, 0x0111, 0x0011, 0x0111,},
        },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmfge_vf_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0][0], res.float32[0], pad.fixu32[0]);
        result_compare_s32(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmfge_vf_32_vm(src0[i].float32[0], src1[i].float32[0][0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmfge_vf_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[1][0], res.float64[0], pad.fixu64);
        result_compare_s64(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vmfge_vf_64_vm(src0[i].float64[0], src1[i].float64[1][0],
