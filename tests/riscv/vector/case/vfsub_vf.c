...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float32 = {
            {0x48bc1e0c, 0x48622627, 0x4800dc08, 0x488d3f87,},
            {0x48bc1e0c, 0x48622627, 0xff7fffff, 0x00000000,},
        },
        .float16 = {
            {0x4f3a, 0x4c58, 0x48f3, 0x4d6d, 0x4ef5, 0x4c4c, 0x4926, 0x4d4c,},
            {0x4f3a, 0x4c58, 0x48f3, 0x4d6d, 0x4ef5, 0x4c4c, 0x4926, 0x0000,},
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        ...
        .float32 = {
            {0x48bc1e0c, 0x11111111, 0x4800dc08, 0x11111111,},
            {0x48bc1e0c, 0x11111111, 0xff7fffff, 0x11111111,},
        },
        .float16 = {
            {0x4f3a, 0x1111, 0x48f3, 0x1111, 0x4ef5, 0x1111, 0x4926, 0x1111,},
            {0x4f3a, 0x1111, 0x48f3, 0x1111, 0x4ef5, 0x1111, 0x4926, 0x1111,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .float64 = {
            ...
            {0x1111111111111111, 0xc0485a626d9fe6ec,},
        },
        .float32 = {
            {0x11111111, 0x48622627, 0x11111111, 0x488d3f87,},
            {0x11111111, 0x48622627, 0x11111111, 0x488d3f87,},
        },
        .float16 = {
            {0x1111, 0x4c58, 0x1111, 0x4d6d, 0x1111, 0x4c4c, 0x1111, 0x4d4c,},
            {0x1111, 0x4c58, 0x1111, 0x4d6d, 0x1111, 0x4c4c, 0x1111, 0x4d4c,},
        },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsub_vf_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[0][0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsub_vf_64_vm(src0[i].float64[0], src1[i].float64[0][0],
