...
{
    int i = 0;
    init_testsuite("Testing insn vwredsum.vs\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_8(vlmax_8 - 1, src0[i].fixs16[0], src1[i].fixs8[0],
                res.fixs16[0], pad.fixs16[0]);
        result_compare_s16(res.fixs16[0], dst_vl[i].fixs16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_8_vm(src0[i].fixs16[0], src1[i].fixs8[0],
                res.fixs16[0], vmo.fixs8, pad.fixs16[0]);
        result_compare_s16(res.fixs16[0], dst_odd[i].fixs16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_8_vm(src0[i].fixs16[0], src1[i].fixs8[0],
                res.fixs16[0], vme.fixs8, pad.fixs16[0]);
        result_compare_s16(res.fixs16[0], dst_even[i].fixs16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_16(vlmax_16 - 1, src0[i].fixs32[0],
                src1[i].fixs16[0], res.fixs32[0], pad.fixs32[0]);
        result_compare_s32(res.fixs32[0], dst_vl[i].fixs32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_16_vm(src0[i].fixs32[0], src1[i].fixs16[0],
                res.fixs32[0], vmo.fixs16, pad.fixs32[0]);
        result_compare_s32(res.fixs32[0], dst_odd[i].fixs32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_16_vm(src0[i].fixs32[0], src1[i].fixs16[0],
                res.fixs32[0], vme.fixs16, pad.fixs32[0]);
        result_compare_s32(res.fixs32[0], dst_even[i].fixs32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_32(vlmax_32 - 1, src0[i].fixs64[0],
                src1[i].fixs32[0], res.fixs64[0], pad.fixs64[0]);
        result_compare_s64(res.fixs64[0], dst_vl[i].fixs64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_32_vm(src0[i].fixs64[0], src1[i].fixs32[0],
                res.fixs64[0], vme.fixs32, pad.fixs64[0]);
        result_compare_s64(res.fixs64[0], dst_even[i].fixs64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsum_vs_32_vm(src0[i].fixs64[0], src1[i].fixs32[0],
                res.fixs64[0], vmo.fixs32, pad.fixs64[0]);
        result_compare_s64(res.fixs64[0], dst_odd[i].fixs64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_8(vlmax_8 - 1, src0[i].fixu16[0], src1[i].fixu8[0],
                res.fixu16[0], pad.fixu16[0]);
        result_compare_u16(res.fixu16[0], dst_vl[i].fixu16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_8_vm(src0[i].fixu16[0], src1[i].fixu8[0],
                res.fixu16[0], vmo.fixu8, pad.fixu16[0]);
        result_compare_u16(res.fixu16[0], dst_odd[i].fixu16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_8_vm(src0[i].fixu16[0], src1[i].fixu8[0],
                res.fixu16[0], vme.fixu8, pad.fixu16[0]);
        result_compare_u16(res.fixu16[0], dst_even[i].fixu16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_16(vlmax_16 - 1, src0[i].fixu32[0],
                src1[i].fixu16[0], res.fixu32[0], pad.fixu32[0]);
        result_compare_u32(res.fixu32[0], dst_vl[i].fixu32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_16_vm(src0[i].fixu32[0], src1[i].fixu16[0],
                res.fixu32[0], vmo.fixu16, pad.fixu32[0]);
        result_compare_u32(res.fixu32[0], dst_odd[i].fixu32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_16_vm(src0[i].fixu32[0], src1[i].fixu16[0],
                res.fixu32[0], vme.fixu16, pad.fixu32[0]);
        result_compare_u32(res.fixu32[0], dst_even[i].fixu32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_32(vlmax_32 - 1, src0[i].fixu64[0],
                src1[i].fixu32[0], res.fixu64[0], pad.fixu64[0]);
        result_compare_u64(res.fixu64[0], dst_vl[i].fixu64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_32_vm(src0[i].fixu64[0], src1[i].fixu32[0],
                res.fixu64[0], vme.fixu32, pad.fixu64[0]);
        result_compare_u64(res.fixu64[0], dst_even[i].fixu64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vwredsumu_vs_32_vm(src0[i].fixu64[0], src1[i].fixu32[0],
                res.fixu64[0], vmo.fixu32, pad.fixu64[0]);
