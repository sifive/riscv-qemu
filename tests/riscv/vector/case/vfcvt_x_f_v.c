...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float32 = {
            ...
            {0x7fffffff, 0x00007970, 0x80000000, 0x00000000,}, 
        },
        .float16 = {
            {0x000e, 0x0002, 0xfffb, 0x0007, 0x000d, 0x0002, 0xfffb, 0x0006,},
            {0x000e, 0x7fff, 0xfffb, 0x8000, 0x000d, 0x0002, 0xfffb, 0x0000,}, 
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        ...
        .float32 = {
            {0x0002d1c8, 0x11111111, 0xfffef448, 0x11111111,},
            {0x7fffffff, 0x11111111, 0x80000000, 0x11111111,}, 
        },
        .float16 = {
            {0x000e, 0x1111, 0xfffb, 0x1111, 0x000d, 0x1111, 0xfffb, 0x1111,},
            {0x000e, 0x1111, 0xfffb, 0x1111, 0x000d, 0x1111, 0xfffb, 0x1111,}, 
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .float64 = {
            ...
            {0x1111111111111111, 0xffffffffffffffdd,},
        },
        .float32 = {
        ...
        .float16 = {
            ...
            {0x1111, 0x7fff, 0x1111, 0x8000, 0x1111, 0x0002, 0x1111, 0x0006,}, 
        },
    },
};
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfcvt_x_f_v_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfcvt_x_f_v_32_vm(src0[i].float32[0], src1[i].float32[0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfcvt_x_f_v_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfcvt_x_f_v_64_vm(src0[i].float64[0], src1[i].float64[0],
