...
int main(void)
{
    ...
    int i, vl;
    init_testsuite("Testing insn vleff\n");

    /* sew 8 */
    test_vleff_8(vlmax_8 - 1, &pad.fixs8[0], src0[0].fixs8[0], res0.fixs8[0]);
    result_compare_s8_lmul(res0.fixs8[0], dst0[0].fixs8[0]);

    test_vleff_8_vm(&vme.fixs8, &pad.fixs8[0], src0[0].fixs8[0], res0.fixs8[0]);
    result_compare_s8_lmul(res0.fixs8[0], dst0[1].fixs8[0]);

    test_vleff_8_vm(&vmo.fixs8, &pad.fixs8[0], src0[0].fixs8[0], res0.fixs8[0]);
    result_compare_s8_lmul(res0.fixs8[0], dst0[2].fixs8[0]);

    /* sew 16 */
    test_vleff_16(vlmax_16 - 1, &pad.fixs16[0], src0[0].fixs16[0], res0.fixs16[0]);
    result_compare_s16_lmul(res0.fixs16[0], dst0[0].fixs16[0]);

    test_vleff_16_vm(&vme.fixs16, &pad.fixs16[0], src0[0].fixs16[0], res0.fixs16[0]);
    result_compare_s16_lmul(res0.fixs16[0], dst0[1].fixs16[0]);

    test_vleff_16_vm(&vmo.fixs16, &pad.fixs16[0], src0[0].fixs16[0], res0.fixs16[0]);
    result_compare_s16_lmul(res0.fixs16[0], dst0[2].fixs16[0]);

    /* sew 32 */
    test_vleff_32(vlmax_32 - 1, &pad.fixs32[0], src0[0].fixs32[0], res0.fixs32[0]);
    result_compare_s32_lmul(res0.fixs32[0], dst0[0].fixs32[0]);

    test_vleff_32_vm(&vme.fixs32, &pad.fixs32[0], src0[0].fixs32[0], res0.fixs32[0]);
    result_compare_s32_lmul(res0.fixs32[0], dst0[1].fixs32[0]);

    test_vleff_32_vm(&vmo.fixs32, &pad.fixs32[0], src0[0].fixs32[0], res0.fixs32[0]);
    result_compare_s32_lmul(res0.fixs32[0], dst0[2].fixs32[0]);

    /* sew 64 */
    test_vleff_64(vlmax_64 - 1, &pad.fixs64[0], src0[0].fixs64[0], res0.fixs64[0]);
    result_compare_s64_lmul(res0.fixs64[0], dst0[0].fixs64[0]);

    test_vleff_64_vm(&vme.fixs64, &pad.fixs64[0], src0[0].fixs64[0], res0.fixs64[0]);
    result_compare_s64_lmul(res0.fixs64[0], dst0[1].fixs64[0]);

    test_vleff_64_vm(&vmo.fixs64, &pad.fixs64[0], src0[0].fixs64[0], res0.fixs64[0]);
    ...
    binv = (void *)((uint64_t)binv + pagesize);
    start = (void *)((uint64_t)brw + pagesize - 16);

    /* sew 8 */
    for (i = 0; i < 16; i++) {
        *((int8_t *)start + i) = src0[0].fixs8[0][i];
    }
    vl = test_vleff_8(vlmax_8, &pad.fixs8[0], start, res0.fixs8[0]);
    result_compare_s8(res0.fixs8[0], dst0[0].fixs8[0]);
    TEST(vl == vlmax_8 / 2);
    /* sew 16 */
    for (i = 0; i < 8; i++) {
        *((int16_t *)start + i) = src0[0].fixs16[0][i];
    }
    vl = test_vleff_16(vlmax_16, &pad.fixs16[0], start, res0.fixs16[0]);
    result_compare_s16(res0.fixs16[0], dst0[0].fixs16[0]);
    TEST(vl == vlmax_16 / 2);
    /* sew 32 */
    for (i = 0; i < 4; i++) {
        *((int32_t *)start + i) = src0[0].fixs32[0][i];
    }
    vl = test_vleff_32(vlmax_32, &pad.fixs32[0], start, res0.fixs32[0]);
    result_compare_s32(res0.fixs32[0], dst0[0].fixs32[0]);
    TEST(vl == vlmax_32 / 2);
    /* sew 64 */
    for (i = 0; i < 2; i++) {
        *((int64_t *)start + i) = src0[0].fixs64[0][i];
    }
    vl = test_vleff_64(vlmax_64, &pad.fixs64[0], start, res0.fixs64[0]);
