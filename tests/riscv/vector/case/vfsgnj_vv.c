...
struct rvv_reg dst_vl[] = {
    {
        ...
        .float32 = {
            {0x4843ca22, 0x483c7bbf, 0xc73b64a7, 0x7f7fffff,},
            {0x4843ca22, 0x483c7bbf, 0xc73b64a7, 0x00000000,},
        },
        .float16 = {
            {0x4b85, 0x4b3d, 0xc332, 0x4a1a, 0x4af1, 0x4aae, 0xc2a4, 0x49a2,},
            {0x7bff, 0x4b3d, 0xfbff, 0x4a1a, 0x4af1, 0x4aae, 0xc2a4, 0x0000,},
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        ...
        .float32 = {
            {0x4843ca22, 0x11111111, 0xc73b64a7, 0x11111111,},
            {0x4843ca22, 0x11111111, 0xc73b64a7, 0x11111111,},
        },
        .float16 = {
            {0x4b85, 0x1111, 0xc332, 0x1111, 0x4af1, 0x1111, 0xc2a4, 0x1111,},
            {0x7bff, 0x1111, 0xfbff, 0x1111, 0x4af1, 0x1111, 0xc2a4, 0x1111,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        .float64 = {
            {0x1111111111111111, 0xc0371fb2129cb102,},
            {0x1111111111111111, 0xc0371fb2129cb102,},
        },
        .float32 = {
...
int main(void)
{
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vv_32(vlmax_32 - 1, src0[i].float32[0],
                src1[i].float32[0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vv_32_vm(src0[i].float32[0], src1[i].float32[0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vv_64(vlmax_64 - 1, src0[i].float64[0],
                src1[i].float64[0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnj_vv_64_vm(src0[i].float64[0], src1[i].float64[0],
