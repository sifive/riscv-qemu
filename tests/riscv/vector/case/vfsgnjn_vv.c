...
struct rvv_reg dst_vl[] = {
    {
        .float64 = {
            {0xc02b982fa8cba1c2, 0x40371fb2129cb102,},
            {0xc02b982fa8cba1c2, 0x0000000000000000,},
        },
        .float32 = {
            {0xc843ca22, 0xc83c7bbf, 0x473b64a7, 0xff7fffff,},
            {0xc843ca22, 0xc83c7bbf, 0x473b64a7, 0x00000000,},
        },
        .float16 = {
            {0xcb85, 0xcb3d, 0x4332, 0xca1a, 0xcaf1, 0xcaae, 0x42a4, 0xc9a2,},
            {0xfbff, 0xcb3d, 0x7bff, 0xca1a, 0xcaf1, 0xcaae, 0x42a4, 0x0000,},
        },
    },
};
...
struct rvv_reg dst_even[] = {
    {
        .float64 = {
            {0xc02b982fa8cba1c2, 0x1111111111111111,},
            {0xc02b982fa8cba1c2, 0x1111111111111111,},
        },
        .float32 = {
        ...
        .float16 = {
            ...
            {0xfbff, 0x1111, 0x7bff, 0x1111, 0xcaf1, 0x1111, 0x42a4, 0x1111,},
        },
    },
};
...
struct rvv_reg dst_odd[] = {
    {
        ...
        .float32 = {
            {0x11111111, 0xc83c7bbf, 0x11111111, 0xff7fffff,},
            {0x11111111, 0xc83c7bbf, 0x11111111, 0xc81eefee,},
        },
        .float16 = {
            {0x1111, 0xcb3d, 0x1111, 0xca1a, 0x1111, 0xcaae, 0x1111, 0xc9a2,},
            {0x1111, 0xcb3d, 0x1111, 0xca1a, 0x1111, 0xcaae, 0x1111, 0xc9a2,},
        },
    },
};
...
{
    int i = 0;
    init_testsuite("Testing insn vfsgnjn.vv\n");


    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_16(vlmax_16 - 1, src1[i].float16[0],
                src0[i].float16[0], res.float16[0], pad.fixu16[0]);
        result_compare_s16_lmul(res.float16[0], dst_vl[i].float16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_16_vm(src1[i].float16[0], src0[i].float16[0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_32(vlmax_32 - 1, src1[i].float32[0],
                src0[i].float32[0], res.float32[0], pad.fixu32[0]);
        result_compare_s32_lmul(res.float32[0], dst_vl[i].float32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_32_vm(src1[i].float32[0], src0[i].float32[0],
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_64(vlmax_64 - 1, src1[i].float64[0],
                src0[i].float64[0], res.float64[0], pad.fixu64);
        result_compare_s64_lmul(res.float64[0], dst_vl[i].float64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vfsgnjn_vv_64_vm(src1[i].float64[0], src0[i].float64[0],
