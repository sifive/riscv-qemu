...
{
    int i = 0;
    init_testsuite("Testing insn vrgather.vi\n");

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_8(vlmax_8 - 1, src0[i].fixs8[0], src1,
                res.fixs8[0], pad.fixs8[0]);
        result_compare_s8_lmul(res.fixs8[0], dst_vl[i].fixs8[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_8_vm(src0[i].fixs8[0], src1,
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_16(vlmax_16 - 1, src0[i].fixs16[0],
                src1, res.fixs16[0], pad.fixs16[0]);
        result_compare_s16_lmul(res.fixs16[0], dst_vl[i].fixs16[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_16_vm(src0[i].fixs16[0], src1,
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_32(vlmax_32 - 1, src0[i].fixs32[0],
                src1, res.fixs32[0], pad.fixs32[0]);
        result_compare_s32_lmul(res.fixs32[0], dst_vl[i].fixs32[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_32_vm(src0[i].fixs32[0], src1,
    ...
    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_64(vlmax_64 - 1, src0[i].fixs64[0],
                src1, res.fixs64[0], pad.fixs64);
        result_compare_s64_lmul(res.fixs64[0], dst_vl[i].fixs64[0]);
    }

    for (i = 0; i < sizeof(src0) / sizeof(struct rvv_reg); i++) {
        test_vrgather_vi_64_vm(src0[i].fixs64[0], src1,
