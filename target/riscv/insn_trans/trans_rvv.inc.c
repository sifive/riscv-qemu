...
#define require_vm(vm, vd) do { if (vm == 0) require(vd != 0); } while(0);
 ...
 * store check function, several rules to be checked:
 *
 * 1. The EMUL setting must within range: 1/8 <= EMUL <= 8 (Section 7.3)
 *
 * 2. Vector register group vector register number must align with
 *    LMUL/EMUL setting. (Section 3.3.2)
 *
 * 3. The EMUL setting must be such that EMUL * NFIELDS ≤ 8. (Section 7.8)
 *
 * 4. Vector register numbers accessed by the segment load and store
 *    cannot increment past 31. (Section 7.8)
 */
#define VEXT_CHECK_STORE(s, vd, nf)              \
 ...
 * load check function.
 */
#define VEXT_CHECK_LOAD(s, vd, nf, vm) \
    VEXT_CHECK_STORE(s, vd, nf);       \
    require_vm(vm, vd);

/*
 * Vector indexed, indexed segment store check function, several rules
 * to be checked:
 *
 * 1. The EMUL setting must within range: 1/8 <= EMUL <= 8 (Section 7.3)
 *
 * 2. Vector register group vector register number must align with
 *    EMUL/LMUL setting. (Section 3.3.2)
 *
 * 3. The EMUL/LMUL setting must be such that EMUL * NFIELDS ≤ 8. (Section 7.8)
 *
 * 4. Vector register numbers accessed by the segment load and store
 *    cannot increment past 31. (Section 7.8)
 */
...
#define VEXT_CHECK_LD_INDEX(s, vd, rs2, nf, vm)     \
    VEXT_CHECK_ST_INDEX(s, vd, rs2, nf);            \
    if (s->emul != s->flmul) {                      \
        require_noover(vd, s->flmul, rs2, s->emul); \
    }                                               \
    if (nf > 1) {                                   \
        require_noover(vd, s->flmul, rs2, s->emul); \
        require_noover(vd, nf, rs2, 1);             \
    }                                               \
    require_vm(vm, vd);

/*
 * Vector AMO check function, server rules to be checked:
 *
 * 1. SEW must be at least as wide as the AMO memory element size.
 *
 * 2. If SEW cannot be greater than XLEN.
 *
 * 3. The EMUL setting must within range: 1/8 <= EMUL <= 8 (Section 7.3)
 *
 * 4. Vector register group vector register number must align with
 *    EMUL/LMUL setting. (Section 3.3.2)
 *
 * 5. Destination vector register group cannot overlap source mask register.
 */
#define VEXT_CHECK_AMO(s, vd, rs2, wd, vm)             \
 ...
 *
 * 1. Vector register numbers are multiples of LMUL. (Section 3.3.2)
 *
